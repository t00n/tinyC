...
main = hspec $ do
    ...
    describe "The generation of three-address-code" $ do
        ...
        it "Generates declarations with complex binary expressions" $ do
            ...
            generateTAC ast `shouldBe` [TACDeclarationValue (TACVar "a") (TACInt 5),TACBinary "t1" (TACVar "a") TACPlus (TACInt 5),TACBinary "t2" (TACVar "a") TACMinus (TACInt 2),TACBinary "t3" (TACVar "t1") TACDivide (TACVar "t2"),TACDeclarationValue (TACVar "b") (TACVar "t3"),TACFunction "tiny" [],TACReturn (TACInt 0)]
        it "Generates function declarations" $ do
            ...
            generateTAC ast `shouldBe` [TACFunction "f" ["a","b"],TACDeclarationValue (TACVar "c") (TACInt 4),TACReturn (TACInt 0),TACFunction "tiny" [],TACReturn (TACInt 0)]
        it "Generates declarations with complex unary expressions" $ do
            let ast = scan_parse_check "int a = 5; int b = -(a - 5); int tiny() {}"
            generateTAC ast `shouldBe` [TACDeclarationValue (TACVar "a") (TACInt 5),TACBinary "t1" (TACVar "a") TACMinus (TACInt 5),TACUnary "t2" TACNeg (TACVar "t1"),TACDeclarationValue (TACVar "b") (TACVar "t2"),TACFunction "tiny" [],TACReturn (TACInt 0)]
        it "Generates function calls" $ do
            let ast = scan_parse_check "int f(int a, int b) { int c = 1; f(c, 2); } int tiny() {}"
            generateTAC ast `shouldBe` [TACFunction "f" ["a","b"],TACDeclarationValue (TACVar "c") (TACInt 1),TACCall "f" [TACVar "c",TACInt 2,TACVar "t1"],TACReturn (TACInt 0),TACFunction "tiny" [],TACReturn (TACInt 0)]
            let ast = scan_parse_check "int f(int a, int b) { int c = f(2 + 3, 1); } int tiny() {}"
            generateTAC ast `shouldBe`  [TACFunction "f" ["a","b"],TACBinary "t1" (TACInt 2) TACPlus (TACInt 3),TACCall "f" [TACVar "t1",TACInt 1,TACVar "t2"],TACDeclarationValue (TACVar "c") (TACVar "t2"),TACReturn (TACInt 0),TACFunction "tiny" [],TACReturn (TACInt 0)]
        it "Generates assignments" $ do
            ...
            generateTAC ast `shouldBe` [TACDeclaration (TACArray "a" (TACInt 5)),TACArrayAccess "t1" (TACArray "a" (TACInt 2)),TACDeclarationValue (TACVar "b") (TACVar "t1"),TACFunction "tiny" [],TACReturn (TACInt 0)]
        ...
        it "Generates if" $ do
            ...
            generateTAC ast `shouldBe` [TACFunction "tiny" [],TACIf (TACExpr (TACInt 1) TACGreater (TACInt 2)) "l1",TACGoto "l2",TACLabel "l1",TACDeclarationValue (TACVar "a") (TACInt 5),TACCopy "a" (TACInt 3),TACLabel "l2",TACReturn (TACInt 0)]
        it "Generates if else" $ do
            let ast = scan_parse_check "int tiny() { if (1 > 2) { int a = 5; } else { int b = 5; } }"
            generateTAC ast `shouldBe` [TACFunction "tiny" [],TACIf (TACExpr (TACInt 1) TACGreater (TACInt 2)) "l1",TACGoto "l2",TACLabel "l1",TACDeclarationValue (TACVar "a") (TACInt 5),TACGoto "l3",TACLabel "l2",TACDeclarationValue (TACVar "b") (TACInt 5),TACLabel "l3",TACReturn (TACInt 0)]
        ...
        it "Generates a while" $ do
            ...
            generateTAC ast `shouldBe` [TACFunction "tiny" [],TACDeclarationValue (TACVar "a") (TACInt 2),TACLabel "l1",TACIf (TACExpr (TACVar "a") TACGreater (TACInt 1)) "l3",TACGoto "l2",TACLabel "l3",TACBinary "t1" (TACVar "a") TACMinus (TACInt 1),TACCopy "a" (TACVar "t1"),TACGoto "l1",TACLabel "l2",TACReturn (TACInt 0)]
        it "Generates a return" $ do
            let ast = scan_parse_check "int tiny() { int a = 2; return a; }"
            generateTAC ast `shouldBe` [TACFunction "tiny" [],TACDeclarationValue (TACVar "a") (TACInt 2),TACReturn (TACVar "a"),TACReturn (TACInt 0)]
        ...
        it "Generates a write" $ do
            ...
            generateTAC ast `shouldBe` [TACFunction "tiny" [],TACDeclarationValue (TACVar "a") (TACInt 2),TACWrite (TACVar "a"),TACReturn (TACInt 0)]
