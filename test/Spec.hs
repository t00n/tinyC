...
main = hspec $ do
    ...
    describe "Parser" $ do
        ...
        it "Parses function call without args" $ do
            ...
            ast `shouldBe` [FuncDeclaration IntType (Variable "tiny") [] (Block [] [Expr $ Call (Variable "lol") []])]
        it "Parses function call with one arg" $ do
            let ast = scan_and_parse "int tiny() { lol(x); }"
            ast `shouldBe` [FuncDeclaration IntType (Variable "tiny") [] (Block [] [Expr $ Call (Variable "lol") [Var $ Variable "x"]])]
        it "Parses function call with several args" $ do
            let ast = scan_and_parse "int tiny() { lol(2, x); }"
            ast `shouldBe` [FuncDeclaration IntType (Variable "tiny") [] (Block [] [Expr $ Call (Variable "lol") [Int 2, Var $ Variable "x"]])]
